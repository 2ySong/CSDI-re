...
class CSDI_base(nn.Module):
    def __init__(self, target_dim, config, device):
        ...
        input_dim = 1 if self.is_unconditional == True else 2  # 如果是无条件就是1，有条件就是2，本配置中是有条件
    ...
    def impute(self, observed_data, cond_mask, side_info, n_samples):
        ...
        for i in range(n_samples):  # 对每个样本执行以下步骤
            # 无条件模型的噪声观测生成
            if self.is_unconditional == True:
...
class CSDI_Forecasting(CSDI_base):
    def __init__(self, config, device, target_dim):
        ...
        # --szy
        # self.ccnet = SEAttention(1)

    def process_data(self, batch):
        observed_data = batch["observed_data"].to(self.device).float()
        observed_mask = batch["observed_mask"].to(self.device).float()
        observed_tp = batch["timepoints"].to(self.device).float()  # observed_tp 是时间点（time points）的缩写，用于表示观测数据的时间戳或时间位置。
    ...
    def forward(self, batch, is_train=1):
        (
            observed_data,  # (B,K,L)：(batch,features,length)
            observed_mask,
            observed_tp,
            gt_mask,
            _,
        ...
        """
        # print("\n1:",observed_data.shape)

        observed_data = observed_data.unsqueeze(1)  # (B,1,K,L)-szy

        # print("2:",observed_data.shape)
        # observed_data = self.ccnet(observed_data)
        observed_data = observed_data.squeeze()  # (B,K,L)-szy
        # print("3:",observed_data.shape)
        """
